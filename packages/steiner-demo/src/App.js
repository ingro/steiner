...
class App extends Component {
  ...
  componentDidMount() {
    const language =
      this.props.settings.language || process.env.REACT_APP_DEFAULT_LANGUAGE;
  ...
  loadTranslations(language) {
    Promise.all([
      import(`vivi/lib/messages/${language}`),
      import(`steiner/lib/messages/${language}`)
    ]).then(([viviMessages, steinerMessages]) => {
      const translations = {
        ...viviMessages.default,
  ...
  render() {
    ...
    return (
      <div>
        ...
        >
          <div>
            {!this.state.isLanguageLoaded && (
              <div className="splash">
                <h1 className="loading dots">{process.env.REACT_APP_NAME} </h1>
              </div>
            )}
            {this.state.isLanguageLoaded && (
              <TranslatorProvider
              ...
              >
                <div>
                  ...
                  >
                    {this.state.isOmniboxOpen && (
                      <Omnibox
                        key="omnibox"
                        onChange={this.toggleOmnibox}
                        options={this.omniboxOptions}
                      />
                    )}
                  </ReactCSSTransitionGroup>
                  <ModalHelp
                  ...
                  >
                    <div
                      className={
                        this.state.isSidebarOpen ? "sidebar-is-open" : ""
                      }
                    >
                      <Header
                        isSidebarOpen={this.state.isSidebarOpen}
